...
export const getFarmsReturnValue = ({
  ...
  const { farmsMapper, liveFarms, liveStakedFarms, finishedFarms, finishedStakedFarms } = farmsCtxState

  const commonToReturn = {
    changeFarmsSubscriptionList,
  }

  const isLiveFarmsLoading = activeSubs[FARMS_DATA_SUB] === FARMS_LIVE_DATA_SUB && liveFarms === null
  const isLiveStakedFarmsLoading = activeSubs[FARMS_DATA_SUB] === FARMS_LIVE_STAKED_DATA_SUB && liveStakedFarms === null
  const isFinishedFarmsLoading = activeSubs[FARMS_DATA_SUB] === FARMS_FINISHED_DATA_SUB && finishedFarms === null
