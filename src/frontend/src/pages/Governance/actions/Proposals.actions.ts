...
import { TezosOperationError, unknownToError } from 'errors/error'
...
export const estimateExecution = async (
  ...
  try {
    ...
    // @ts-ignore
    const operationEstimate = await tezos?.estimate.transfer(tezosOperation, 1)
    return operationEstimate
  } catch (e) {
    const unknownError = unknownToError(e)
    const tezosError = new TezosOperationError(unknownError)

    console.error('estimateExecution error', e)

    const errorCode = Number(tezosError.message) ? Number(tezosError.message) : null
