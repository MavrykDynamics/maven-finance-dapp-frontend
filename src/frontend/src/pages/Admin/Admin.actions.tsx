...
export const testErrorCodesChangeAdmin = (accountPkh?: string) => async (dispatch: AppDispatch, getState: GetState) => {
  const state: State = getState()
  ...
  try {
    ...
    console.log('logging operation estimate:', operationEstimate)
    const operation = await contract.methods.setAdmin('tz1byTGaUKjJqkwSXPnM3dpf9N39pYwRfnTm').send()
    const done = await operation?.confirmation()
    console.log('done', done)
    dispatch(showToaster(SUCCESS, 'Tracking Farm done...', 'All good :)'))
  } catch (error) {
